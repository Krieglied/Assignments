...
client.close()
output = struct.unpack("!LL", received__packed_string)
print(output)
client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
print("SOCKET IS REOPENED")
client.bind(('0.0.0.0', accept_port_2))
data = cmd_list['get_key']
packed_string = struct.pack("!LL", data, output[0])
client.connect(dest)
client.send(packed_string)
received__packed_string = client.recv(1024)
client.close()
output = struct.unpack("!32s", received__packed_string)
