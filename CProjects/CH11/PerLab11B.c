...
char * split_the_string(char * string_ptr, char delimiter)
{
	...
	//This line accounts for string_ptr if delimiter is not found
	char * return_ptr = string_ptr;
	int counter = 0;
	int iterator = 0;
	int position = 0;
	//Iterate through string
	while(*(string_ptr + iterator) != '\0')
	{
		//If string element equals the provided delimiter
		if (*(string_ptr + iterator) == delimiter)
		{
			//And this is the first instance of the delimiter
			if (counter == 0) {
				//Set the return ptr to the element after the current one
				return_ptr = &*(string_ptr + iterator + 1);
				//And increment the counter by one
				counter++;
				position = iterator;
			}
			//If the this is not the first instance of the delimiter, then ERROR_ABUNDANT_DELIMITER needs to be returneds
			else
			{
	...
	//If a delimiter was found, then the original string needs to be null-terminated at the position of the delimiter
	if (counter != 0)
	{
		*(string_ptr + position) = '\0';
	}
	//If there was only one delimiterm then return_ptr will be equal to the string starting at the delimiter+1 position
	//If there was no delimiter, then return_ptr will simply be the original string_ptr
	return return_ptr;
}
